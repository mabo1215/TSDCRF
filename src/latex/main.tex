...
Figure~\ref{fig:14}, Figure~\ref{fig:15}, Figure~\ref{fig:16} and Figure~\ref{fig:17} show the detection of tracked objects by different methods. The  $k=1,k=5,k=10,k=15,k=20,k=25$ are the numbers of classes added to the test separately. We use the frequency with which privacy classes are retrieved by an attacker as a measure of privacy leakage: the higher the retrieval frequency, the worse the privacy protection (i.e., the lower the level of protection). To compare the actual situation, we \el{ have made two extreme cases in the test, one is|can still be detected at 0, the data has some privacy classes that are the same as the normal data, and the training data themselves have many repetitions but different labels. When the budget is 0, and the frequency is not 0, it means that the same data marked in this dataset have different labels, and the different labels have privacy classes and non-privacy classes. At the same time, before the experiment, we had extracted the required target detection category and corresponding image via} retrieving the data category. This process is called cleaning data. 
